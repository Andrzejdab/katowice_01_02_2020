...
class TestPremiumEmployee:
    def test_init(self):
        pe = PremiumEmploee("Jan", "Nowak", 100.0)
        assert pe.first_name == "Jan"
        assert pe.last_name == "Nowak"
        assert pe.rate_per_hour == 100.0

    def test_give_bonus(self):
        pe = PremiumEmploee('Jan', 'Nowak', 100)
        assert pe.pay_salary() == 0
        pe.give_bonus(1000)
        assert pe.pay_salary() == 1000
        assert pe.pay_salary() == 0



    def test_pay_salary_many_days_bonus(self):
        pe = PremiumEmploee('Jan', 'Nowak', 100)
        pe.register_time(10)
        pe.register_time(10)
        pe.register_time(10)
        pe.give_bonus(1000)
        assert pe.pay_salary() == 3 * 8 * 100 + 3 * 2 * 200 + 1000